...
provides image file reading and writing (`cv::imread`/`cv::imwrite` and
related functions). This baseline build has every external codec library
...
`lib{opencv-imgcodecs}` is the OpenCV imgcodecs module: image file reading
and writing.
