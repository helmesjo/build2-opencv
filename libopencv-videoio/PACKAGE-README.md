...
provides video I/O: reading and writing video, image sequences, and camera
streams through `cv::VideoCapture`/`cv::VideoWriter`. This baseline build has
every external capture/encode backend (V4L, FFmpeg, GStreamer, DirectShow,
Media Foundation, AVFoundation, and others) off, leaving the built-in
image-sequence (`CAP_IMAGES`) and MJPEG/AVI container (`CAP_OPENCV_MJPEG`)
support, which need no external library.
